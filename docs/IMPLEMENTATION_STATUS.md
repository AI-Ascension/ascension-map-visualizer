...
Installed client `0.153.4` accepted explicit `gpt-5.6-luna` / `max` settings for eight original descendants and three resumed tasks. Independent runtime metadata confirmed that pair for all eleven, each directly under the root at depth one. The original agents were absent from the native registry before resumed tasks were assigned. The root remained `gpt-6-astra`. The peak of eight descendants stayed below the project ceiling of twelve.
