...
for (const entry of manifest.files) {
  if (!/^[0-9a-f]{40}$/.test(entry.commit)
      || !/^(contract-artifacts\/harness|fixtures\/(demo|conformance))\/[a-z-]+(?:\.schema)?\.json$/.test(entry.copy)
...
process.stdout.write(`Verified ${copies.size} inert owner artifacts across ${commits.size} source commits.\n`);
