...
export function validateManifest(raw, root = repositoryRoot) {
  ...
  for (const entry of raw.files) {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry) || Object.keys(entry).length !== 4
      || !/^[0-9a-f]{40}$/.test(entry.commit) || !/^[a-f0-9]{64}$/.test(entry.sha256)) fail('file entry schema');
...
// One tree cannot vouch for every copy: each entry is verified against its own immutable owner revision.
const revisions = [...new Set(manifest.files.map((entry) => entry.commit))].sort();
...
try {
  ...
  for (const revision of revisions) {
    ...
    if (execFileSync('git', ['-C', checkout, 'rev-parse', 'HEAD'], {encoding: 'utf8'}).trim() !== revision) {
      throw new Error(`historical checkout did not resolve pinned commit ${revision}`);
    }
    for (const entry of manifest.files.filter((candidate) => candidate.commit === revision)) {
  ...
  process.stdout.write(`Verified ${manifest.files.length} historical harness files at ${revisions.length} immutable harness revisions: ${revisions.join(', ')}.\n`);
} finally {
